...
$string['descriptiontxt'] = '<p>プロファイルリダイレクトブロックは、ブロックを含むサイトのフロントページまたはマイMoodleページから新しいコースにユーザをリダイレクト
するように設計されています。ユーザがリダイレクトされる新しいコースは、以下のサイト構成に基づくコースのフルネーム、省略名、またはID番号の値と一致するプロファイルフィールドの値に基づいています。</p>

<p>ブロック設定が完了したら、管理者はブロックのインスタンスをフロントページまたはマイMoodleページに追加する必要があることに注意してください。
これを行うには、次の手順を実行します。
<ol>
<li>フロントページに移動して、編集を有効にし、[ブロックを追加（Add a Block）]ブロックドロップダウンから[プロファイルリダイレクト]を選択します。</li>
<li>ブロックが追加されると、すべてのユーザはプロファイルフィールドで指定されたコースにリダイレクトされます。プロファイルに有効な値がない場合は
デフォルトのコースにリダイレクトされます。</li>
</ol>
</p>';
$string['profilefield'] = 'プロファイルフィールド';
$string['profilefielddesc'] = 'これは、コースフィールドを照合するために、プロファイルリダイレクトブロックで使用されるユーザ
プロファイルのフィールドです。';
$string['coursefield'] = 'コースフィールド';
$string['coursefielddesc'] = 'これは、リダイレクトでユーザのプロファイルフィールドを照合するために、プロファイルリダイレクトブロックで
使用されるコース設定のフィールドです。';
